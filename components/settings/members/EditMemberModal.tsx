...
export default function EditMemberModal({ isOpen, isDark, member, onClose, onSave, onUpdate }: EditMemberModalProps) {
  if (!isOpen || !member) return null

  const handleUpdateField = <K extends keyof Member>(field: K, value: Member[K]) => {
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {roles.map((role) => (
                  <button
                    key={role.key}
                    onClick={() => handleUpdateField("role", role.key as Role)}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {departments.map((dept) => (
                  <button
                    key={dept.key}
                    onClick={() => handleUpdateField("department", dept.key)}
                    className="flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm transition-colors"
                    style={{
                      borderColor:
                        member.department === dept.key
                          ? cortifactColors.accent
                          : isDark
                          ? cortifactColors.border.dark
                          : cortifactColors.border.light,
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {authorityLevels.map((level) => (
                  <button
                    key={level.key}
                    onClick={() => handleUpdateField("authorityLevel", level.key as "Low" | "Medium" | "High")}
                    className="flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm transition-colors"
                    style={{
                      borderColor:
                        member.authorityLevel === level.key
                          ? cortifactColors.accent
                          : isDark
                          ? cortifactColors.border.dark
                          : cortifactColors.border.light,
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {["Active", "Pending"].map((status) => (
                  <button
                    key={status}
                    onClick={() => handleUpdateField("status", status as Status)}
                    className="flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm transition-colors"
                    style={{
                      borderColor:
                  ...
                  >
                    <span
                      ...
                      style={{
                        backgroundColor:
                          status === "Active" ? cortifactColors.success : cortifactColors.warning,
                      }}
                    />
                    {status}
                  </button>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleUpdateField("specialization", e.target.value)}
